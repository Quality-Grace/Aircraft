...
public class FontStyle {
	...
	/// this specifies the x and y offsets for each binary value in the "shadow
	/// location byte", and is what causes each value to progress in a circle.
	private static int[] shadowPosMap = { 0, 1, 1, 1, 0, -1, -1, -1, -1, -1, 0, 1, 1, 1, 0, -1 };
	/**
	 * The shadowing technique uses binary strings to specify where to outline a
	 * string of text. It was going to be a straight byte, since there are 8
	 * positions, but since it's going to be a string anyway, I decided to make it
	 * accept a string. Each position is specified fairly simply: it goes clockwise,
	 * starting from the top. Then it goes top right, right, bottom right, bottom,
	 * etc. up to top left. It's kind of like a compass, with a position for N, NE,
	 * E, etc. For an example, for the default shadow, the string is "00010000".
	 * though, because of the way it's designed, the trailing zeros may be dropped,
	 * so it could just be "0001". This doesn't quite read like binary, but it
	 * doesn't have to, so whatever. :P
	 */
	...
	private RelPos relLinePos = RelPos.CENTER; // when setup for a paragraph with multiple lines, this determines the
	// alignment of each line within the bounds of the paragraph.
	...
	/*
	 * -- All the font modifier methods are below. They all return the current
	 * FontStyle instance for chaining. --
	 */
	...
	/**
	 * sets the x position of the text anchor. This causes the text to be
	 * left-justified, if alignment is reset.
	 */
	...
	/**
	 * sets the y position of the text anchor. This sets the y pos to be the top of
	 * the block, if alignment is reset.
	 */
	...
	/**
	 * This enables text shadowing, and sets the shadow color and type. It is a
	 * convenience method that offers a preset for text outlines, and a single
	 * shadow in a standard direction.
	 */
	...
	/**
	 * This is what acutally sets the values described above. It also allows custom
	 * shadows.
	 */
